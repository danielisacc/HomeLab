...
#### Disclaimer
The DC is fully functioning now with Active Directory and is also acting as the Default Gateway. This is a HomeLab, and that is the only reason this one server is acting as both, but in a real company, this would be an unnecessary headache as the DC acting as a Gateway and an AD would cause many issues for the AD. This is also makes our network very vulnerable to threats, since the central controller of our network is also the first thing interacting with the internet. A simple DoS attack could drop our entire network. I wanted practice working with Active Directory, but I also wanted practice with DHCP and RAS/NAT services, which is why I ran both services on the same DC.


## PowerShell Scripting
Now that the AD is fully functioning, we will explore methods of populating our network with users. Although it may seem more straightforward to manually enter every user into the AD like we did before, one at a time, but we may need to enter in hundreds or even thousands of users for a newly created AD. What if we were just hired as the Network Engineer for a company that has never had an AD and you need to create their network from scratch. How would you enter in possibly 30, 60, or even 200 user's into your AD? Well, PowerShell is an interfact that will make this process much faster and easier, but you need to know how to use it properly, otherwise it may be impossible. 
### What is PowerShell Scripting
PowerShell is a Windows command line shell, meaninging it's a language used to automate tasks from within a Windows Command Line Interface (CLI), which is an "Interface" the user can use to execute commands to the OS. So what is PowerShell Scripting? Well, executing one command from within the CLI is helpful, and you can create a user with one command, but to create a user there are a lot of parameters to enter for a one line command, and more importantly, you are still creating one user at a time. Well, since CLI is based in text, what if you could create a document that acts as a framework you can apply to any user with only a couple key strokes? What if you could make it so that with one, simple command, you could create thousands of users? Well, that is what Scripting is. A script is an executable file that can contain multiple commands from within it, including variables, which allows automation that acts as a framework. This section describes how to create and execute a PowerShell script.
### Creating Our Script
Before creating any scripts, you need to open PowerShell, specifically ISE, since it's interface allows user's to quickly create a script and execute live commands. To open it, press the windows key and type *PowerShell ISE* and select the matching option displayed. This opens the *PowerShell ISE* window. There are two panes within the window. One is the script creation tool and the other is the PowerShell CLI. We will start from within the script
